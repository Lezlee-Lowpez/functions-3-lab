...
// const dividedByTwo = numbers.forEach(element => console.log(element/2))

// console.log(dividedByTwo) /// how to return a forEAch to console.log outside of the function
...
// const newDividedByTwo = numbers.map(element => (element/2))

// console.log(newDividedByTwo)
...
// CODE HERE
// const cityWithI = cities.filter(arr => arr.endsWith("i"))
...
const fruitTotal = fruits.forEach((fruit, index) => {
  if (fruit.color === "red"){
    ...
    console.log(`The fruit with index ${index} is an neither an apply or orange.`)
  }
})
...
// so we have an array called foods and each index has an object with name, price, and tags. 
// so we first need to check if the tags key includes rice -->sounds like the filter 
// if it does: then we can take the price key and add them all up THE ANSWER SHOULD BE 36



// const totalSumOfRice = foods.filter(value => value.tags.includes("rice")).reduce((acc, curr)=> acc + curr.price, 0)

// console.log(totalSumOfRice)
